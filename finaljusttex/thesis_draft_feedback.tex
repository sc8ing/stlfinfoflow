...
\end{verbatim}
 ...
 \coqdocvar{High} or \coqdocvar{Low}. The assertion that the flow of
 information in a program is secure can then be stated as follows:
 for any two sets of high-security inputs \coqdocvar{H1},
 \coqdocvar{H2} and low-security input \coqdocvar{L}, the
 low-security result of executing a program with \coqdocvar{H1} and
 \coqdocvar{L} as its initial states is identical to executing it
 with \coqdocvar{H2} and \coqdocvar{L} as its initial states. This
 property is referred to as non-interference. \joe{I would cite some
 of the classic papers on information flow in which this definition
 is introduced.}
...
Automation (INRIA). \joe{I think the presentation was to the Oregon
Programming Languages Summer School. INRIA is Francois's affiliation.
I would cite the slides here, as well as the paper we looked at.}
...
\jacob{Is this what you mean below?}
...
context \coqdocvar{Gamma}. When a label \coqdocvar{l} is introduced
into the type like $T^l$, this means that the expression belongs to
the security class \coqdocvar{l}.
...
\begin{prooftree}
    ...
    \UnaryInfC{$\Gamma\vdash$ marked $l$ body : $T^l$}
\end{prooftree}
...
\item  Marked expressions are only slightly more complex. Given the
    stepping relation and the assumption that \coqdocvar{marked}
        \coqdocvar{class} \coqdocvar{body} --> \coqdocvar{f}, a few
...
more possibilities as to how \coqdocvar{e} --> \coqdocvar{f}. While
...
\item  Now assume the step from \coqdocvar{e} to \coqdocvar{f} was
        ...
        \coqdocvar{arge'} -->* \coqdocvar{marked} \coqdocvar{class}
...
\coqdocvar{e} --> \coqdocvar{m} and \coqdocvar{m} -->* \coqdocvar{f}.
...
the form \coqdocvar{e} -->* \coqdocvar{m} and \coqdocvar{m} -->
