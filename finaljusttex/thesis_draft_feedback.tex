...
context \coqdocvar{Gamma}. The notation $l \triangleleft Bool^{l'}$ is a proposition that's true if $l \leq l'$ for security classes $l$ and $l'$.
...
security label $l$ steps to some \coqdocvar{value} $v$, the prune of
...
\coqdocvar{T\_marked}). Consequently, pruning $v$ as specified is
$v$. Overall, we now have \coqdocvar{noholes} $v$, $\prune{v}$ = $v$,
...
have \coqdocvar{noholes} $v$, $\prune{e_1}\to_* v$, and $\prune{e_1}
<< \prune{e_2}$ (reflexivity of \coqdocvar{holier}) – the three
