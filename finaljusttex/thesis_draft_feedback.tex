...
voted. (TODO better example?). \joe{The only problem with this example is
  that it would not work with the strict information flow system considered here, because the aggregate total is derived from individual votes.} In such cases it's not only critical
...
non-interference. \joe{I would say ``property'' not ``proposition''}. TODO: how to state this property for the way this
language is set up? 
\joe{I would cite some of the classic papers on information flow in which this definition is introduced.}
...
\joe{You can include a citation to Coq itself here.}
...
\joe{I would say a little bit more about how you generated this document: it is literally derived from the machine checked proofs, so that when \texttt{Lemma} and the like occur below, the reader should understand that these are verbatim statements from Coq that have been checked.}
...
Science and Automation (INRIA). \joe{I think the presentation was to the Oregon Programming Languages Summer School. INRIA is Francois's affiliation. I would cite the slides here, as well as the paper we looked at.} Some modifications have been made for
...
used is that it must be typed. (TODO: is this true?). \joe{I would simply delete this last sentence.} \begin{coqdoccode}
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
\subsection{Syntax}
...
\joe{Could you briefly give some intuition for holes here, as you have done for marked?}
...
\coqdocvar{e'}''. \joe{The way coqdoc formats the e' makes it hard to distinguish between the ' and the end of the quotation marks.} The formal rules are detailed below. 
...
\joe{shouldn't it be is v, not is s?}
...
\coqdocemptyline
\end{coqdoccode}
\joe{You might make mention of the fact that you will only use this substitution operation when v is a closed expression -- otherwise you would have to worry about capture-avoiding substitution.}
...
\joe{The \texttt{-->} looks kind of ugly in the latex. Is there a way to get Coqdoc to translate this with a unicode symbol (I believe there is).}
...
\joe{I would say something about the Lift rules -- point out that the other rules are pretty standard, but these ones are different because they're needed to make marked work properly.}


\subsection{Typing}


\joe{We discussed this on Friday, but the types for marked have to incorporate the label. }
...
expressions. \joe{The wording of the previous sentence is kind of awkward.} A variety of propositions and lemmas used to construct
...
\joe{The title of this subsection is confusing because we haven't seen yet what monotonicity is.}
...
holey`` expression and the relationship between expressions with
    ...
    seeks to provide.
    \joe{Quote mark around holey is messed up.}
...
\joe{I would say ``We also define a proposition noholes...'' rather than ``Also given...''}
...
a value entails that \coqdocvar{v'} is also a value. \joe{I would say ``have the same shape'' rather than ``requires the congruence.} The proof proceeds by
...
them. 
\joe{This paragraph only makes sense if you give a little bit more detail on Coq and tactics in the background section.}
...
never used. \joe{Latex quote marks are reversed here.}
...
expression \coqdocvar{e'} must be equivalent to \coqdocvar{e'}. \joe{I would delete ``the way relativity is defined'' and just say ``The way the holier relation is defined means that...''} The proof proceeds by
...
equivalence for expressions with sub-expressions. 
 ...
 As an example, the \coqdocvar{e} = \coqdocvar{test} \coqdocvar{cond} \coqdocvar{b1} \coqdocvar{b2} case is given. \joe{I would say ``consider the ... case'' rather than ``... case is given''} From the
...
\joe{You could use ``Axiom'' for the lemma that is unproven in your development.}
...
\joe{I would delete ``which is disappointing''}
...
\joe{In coq, didn't you define it as a function? If so, I would actually just give the literal code for implementing it, rather than this formulation as an inductive definition by rules -- I'm not sure the version you've written down here actually prunes multiple labels correctly.}
...
\item  The idea for using stability and monotonicity to complete
  ...
  Pottier's 2004 presentation at INRIA. \joe{See remark earlier about this.} The slides of his
...
\item  TODO: any other things I should add?
  ...
  \joe{The basic literature on information flow, like the other paper you read and some of the sources cited there.}
